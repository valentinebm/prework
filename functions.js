...
function findLongestWord( array_words ){
		...
		for (var i=0; i<array_words.length; i++) {
			if (array_words[i].length > length) {
				var length = array_words[i].length
...
{ var sum = 0; for (var i = 0, length = array.length; i < length; i++)
	sum += array[i]
	return( sum/length )
}
...
function charFreq( string ) {
	...
	for (var i = 0; i < string.length; i++){
		if (!freqCounter.hasOwnProperty(string[i])) {
